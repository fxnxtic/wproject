...
class RedisContextStorage(IContextStorage):
    ...
    async def get_messages(self, key, with_tokens=False):
        raw = await self.redis.lrange(kb.messages(key), 0, -1)
    ...
    async def add_message(self, key, message: dict[str, Any]):
        ...
        await self.redis.rpush(kb.messages(key), json.dumps(message))
        ...
        if removed and self.config.CONTEXT_SUMMARIZE:
            ...
            base = (
                [{"role": ContextRole.SYSTEM, "content": as_block(summary)}]
                if summary
                else []
            )
